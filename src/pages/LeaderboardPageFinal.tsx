...
const BADGE_COLORS: Record<string, string> = {
  'MITO': 'bg-yellow-500 text-black',
  'NÃO MITO': 'bg-red-600 text-white',
  'PFTO': 'bg-blue-500 text-white',
  'Café': 'bg-amber-700 text-white',
  'Master': 'bg-purple-600 text-white',
  'CIBED': 'bg-green-600 text-white',
  'Island': 'bg-cyan-500 text-white',
  'Empreg': 'bg-gray-600 text-white',
  'Pulta': 'bg-pink-500 text-white',
  'Condado': 'bg-red-400 text-white',
};

function BadgeTag({ badge, icon }: { badge: string; icon?: string }) {
  const colorClass = BADGE_COLORS[badge] || 'bg-gray-500 text-white';
  const display = icon ? `(${icon})` : `(${badge})`;
  return <span className={`ml-1 text-xs px-1 rounded ${colorClass}`}>{display}</span>;
}
                  ...
                  <div key={player.id} className={`flex items-center gap-3 px-4 py-3 ${isUser ? 'bg-primary/5' : ''}`}>
                    ...
                    <div className="flex-1">
                      <p className={`font-bold text-sm ${isUser ? 'text-primary' : 'text-foreground'}`}>
                        ...
                        {player.badges?.includes('Café') && <BadgeTag badge="Café" />}
