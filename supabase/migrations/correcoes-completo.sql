-- ============================================
-- SQL DE CORREÇÕES COMPLETO
-- Execute todo este código no SQL Editor do Supabase
-- ============================================

-- 1. CORRIGIR TIPOS DE ATIVIDADE (activity -> multiple_choice)
    ...
    CASE 
      WHEN (step->>'type') = 'activity' THEN
        jsonb_build_object(
          ...
          'question', COALESCE(step->>'question', ''),
          'options', step->'options',
          'correct', (step->>'correct')::integer,
...
BEGIN
  ...
  -- Check streak
  v_is_new_day := v_profile.last_lesson_date IS DISTINCT FROM v_today::text;
...
SELECT 
  ...
  title, 
  xp_reward,
  jsonb_array_length(activity_data) as total_steps
FROM public.lessons 
ORDER BY section_id, lesson_number
LIMIT 20;

-- Verificar se funções foram criadas
SELECT proname, pronargs FROM pg_proc WHERE pronname IN ('complete_lesson_v2', 'lose_heart_v2');